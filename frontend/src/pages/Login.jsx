...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      dispatch(setCredentials({ user: response.data.user, access: response.data.access }));
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    try {
      const res = await axios.post('http://localhost:8000/api/auth/google/', { credential: credentialResponse.credential });
      dispatch(setCredentials({ user: res.data.user, access: res.data.access }));
      localStorage.setItem('refresh_token', res.data.refresh);
      toast.success(res.data.created ? `Welcome to Liora, ${res.data.user.username}! 🎉` : `Welcome back, ${res.data.user.username}! 👋`);
      navigate('/');
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#FAFAFA] px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-6 md:mb-8">
          <h1 className="text-3xl md:text-4xl font-serif font-light text-gray-900 tracking-wide mb-2">Liora</h1>
          <p className="text-gray-500 text-sm">Welcome back</p>
        </div>

        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6 md:p-8">
          <h2 className="text-lg md:text-xl font-medium text-gray-900 mb-5 md:mb-6 text-center">Login</h2>
