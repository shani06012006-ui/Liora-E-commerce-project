        ...
        try:
            ...
            items = (
                OrderItem.objects.filter(
                    order__created_at__gte=start_date, order__status="delivered"
                )
    ...
    def get(self, request):
        try:
            ...
            users = User.objects.filter(
                orders__created_at__gte=start_date, orders__status="delivered"
            ).distinct()
            ...
            if segment == "new":
                ...
                users = users.annotate(order_count=Count("orders")).filter(
                    order_count__gt=1
                )
            elif segment == "high":
                users = users.annotate(total_spent=Sum("orders__total_amount")).filter(
                    total_spent__gt=1000
                )

            result = []
            for user in users[:20]:
                user_orders = user.orders.filter(
                    created_at__gte=start_date, status="delivered"
                )
                total_orders = user_orders.count()
                total_spent = (
                    user_orders.aggregate(total=Sum("total_amount"))["total"] or 0
                )
    ...
    def get(self, request):
        try:
            ...
            for product in products:
                ...
                total_revenue = float(
                    order_items.aggregate(total=Sum(F("price") * F("quantity")))[
                        "total"
                    ]
                    or 0
                )
                ...
                previous_sales = (
                    previous_items.aggregate(total=Sum("quantity"))["total"] or 0
                )
                trend = 0
                if previous_sales > 0:
