...
class AdminSettingsSectionView(APIView):
    ...
    def put(self, request, section):
        ...
        settings_obj.save()
        return Response(
            {"message": "Settings saved successfully", "data": request.data}
        )
...
class AdminSettingsArrayItemView(APIView):
    ...
    def post(self, request, section, array_field):
        ...
        if not isinstance(arr, list):
            return Response(
                {"error": f"{array_field} is not a list field"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if value not in arr:
            arr.append(value)
        ...
        settings_obj.save()
        return Response(
            {"message": "Item added", "data": current}, status=status.HTTP_201_CREATED
        )
