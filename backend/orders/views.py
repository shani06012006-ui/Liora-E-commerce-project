...
class CartView(APIView):
    ...
    def post(self, request):
        ...
        if product.stock < quantity:
            return Response(
                {"error": f"Only {product.stock} left in stock."}, status=400
            )
    ...
    def put(self, request, item_id):
        ...
        if item.product.stock < new_qty:
            return Response(
                {"error": f"Only {item.product.stock} left in stock."}, status=400
            )
...
class AdminOrderListView(APIView):
    ...
    def get(self, request):
        ...
        if payment_method_filter:
            orders = orders.filter(payment_method__iexact=payment_method_filter)
