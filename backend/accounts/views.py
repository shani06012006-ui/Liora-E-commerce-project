...
class SetDefaultAddressView(APIView):
    ...
    def post(self, request, address_id):
        ...
        return Response(
            {"message": "Default address updated."}, status=status.HTTP_200_OK
        )
            ...
            if existing_user:
                if existing_user.is_active and not existing_user.is_deleted:
                    ...
                    OTPVerification.objects.create(
                        user=existing_user, otp_code=otp_code
                    )
                    send_otp_email.delay(
                        existing_user.email, otp_code, existing_user.username
                    )
...
class VerifyOTPView(APIView):
    ...
    def post(self, request):
        ...
        otp_obj = (
            OTPVerification.objects.filter(user=user, otp_code=otp_code)
            .order_by("-created_at")
            .first()
        )
