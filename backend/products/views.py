        ...
        try:
            ...
            try:
                ...
                revenue_data = Order.objects.filter(
                    created_at__gte=last_30_days, status="delivered"
            ...
            try:
                ...
                top_categories_data = [
                    {"name": cat.name, "count": 0} for cat in top_categories
                ]
            ...
            low_stock_products_data = ProductSerializer(
            ...
            try:
                ...
                if revenue_period == "week":
                    ...
                    daily = (
                        RevenueOrder.objects.filter(
                            created_at__gte=start, status="delivered"
                        )
                    ...
                    revenue_by_day = {
                        row["day"]: float(row["revenue"] or 0) for row in daily
                    }
                    for i in range(7):
                    ...
                    revenue_by_month = {
                        row["month"].month: float(row["revenue"] or 0)
                        for row in monthly
                    }
                    months_short = [
                        "Jan",
            ...
            return Response(
                {"error": str(error), "message": "Failed to load dashboard data."},
                status=500,
            )
